...
import { TParseOperation } from '@sinclair/typebox/value';
...
const designRegistry = bucket(new WeakMap<object, {
    paramtypes: ParamTypes;
    getParamTypes(propertyKey: string | symbol): ParamTypes;
}>(), (target: object) => {
    return {
        paramtypes: Array.from(Reflect.getOwnMetadata('design:paramtypes', target) ?? { length: 0 }, ParamType),
        getParamTypes: bucket(new Map<string | symbol, ParamTypes>(), (propertyKey) => {
...
function getParamTypes(target: object, propertyKey: string | symbol | undefined) {
    return typeof propertyKey === 'undefined' ?
        designRegistry(target).paramtypes :
        designRegistry(target).getParamTypes(propertyKey);
}
...
function construct(constructor: Function): any {
    ...
    const design = designRegistry(constructor).paramtypes;
